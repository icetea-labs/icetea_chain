...
class LuckyBot extends SurveyBot {
  ...
  @transaction oncommand_result() {
    ...
    winners.forEach(({ number, telegram, timestamp, delta }, index) => {
      reply += `<br>${index +
        1}. <a href='https://t.me/{telegram}' target='_blank'>@${telegram}</a> - ${number} (±${delta}) - ${formatTime(
        timestamp
      )}`;
    });
  ...
  intro() {
    ...
    return Message.html(
      '<b>NOTE</b><br/> The game is dedicated for member of Icetea Vietnam Telegram group <a href="https://t.me/iceteachainvn" target="_blank">@iceteachainvn</a> only. Please join our group before entering this game.'
    )
  ...
  validate_number({ text, chatData }) {
    ...
    if (value.length !== 3 || !/\d\d\d/.test(value)) {
      throw new Error(
        "Please input a valid number with 3 digits, e.g 012, 123."
      );
    }
  ...
  after_number() {
    ...
    return Message.html(
      "Step 2: Please input your Telegram username.<br/>(Both <b>@username</b> or just <b>username</b> are accepted)"
    ).input("@username");
  }
  ...
  after_telegram({ chatData }) {
    ...
    return Message.html(reply).button("Confirm", "confirm");
  }
  ...
  after_confirm({ chatData }) {
    ...
    return Message.text(
      `Thank you for your participation!<br/> The final results will be announced on Friday, ${deadlineText}. Good luck :D`
    );
  }
}
