...
exports.randomAccountWithBalance = async (tweb3, intialBalance = 10000) => {
  ...
  await tweb3.wallet.importAccount(configKey)
  // const from = account.account +'a'
  const keyInfo = await ecc.newKeyPairWithAddress()

  // send money from configKey to newKey
  const result = await tweb3.sendTransactionCommit({ from: 'tea_3M76NHiZ4ipvMmTVBVJ1Wj378RLJ', to: keyInfo.address, value: intialBalance })
