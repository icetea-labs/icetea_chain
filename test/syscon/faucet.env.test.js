...
describe('faucet', () => {
  test('test request', async () => {
    ...
    const ev = r.events.filter(e => e.eventName === 'FaucetTransferred')
    expect(ev.length).toBe(1)
    expect(ev[0].emitter).toBe('system.faucet')
    expect(ev[0].eventName).toBe('FaucetTransferred')
    expect(String(ev[0].eventData.amount)).toBe(REQUEST_QUOTA_S)
    expect(ev[0].eventData.requester).toBe(addr2)
