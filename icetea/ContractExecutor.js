...
module.exports = class ContractExecutor {
    ...
    invoke(invokeType, contractAddress, methodName, methodParams, options) {
        ...
        if (methodName === 'address') {
            ...
            return utils.balanceOf(contractAddress, this.stateTable)
        }

        const { mode, src } = getContractInfo()
        ...
        const result = vm.run(src, { context, guard })

        if (invokeType === 'transaction') {
            // save back the state
            this.stateTable[contractAddress].state =
                Object.assign(this.stateTable[contractAddress].state, context._state)
    ...
    queryMetadata(contractAddress) {
        return this.invoke('metadata', contractAddress)
    }
    ...
    async execTx(tx) {
        ...
        // Free block to ensure readonly access by contract
        const freezedBlock = Object.freeze(this.lastBlock)
